...
class irtpvc_augmented(Dataset):
    def __init__(self, split='train', augment_factor=500, train_ratio=26, val_ratio=6, test_ratio=6):
        self.sequence_dir = args.data_folder + '/augmented_dataset/'
        self.gt_dir = args.data_folder + '/labels/auto/'
        self.sequence_folders = sorted(os.listdir(self.sequence_dir))
    ...
    def __len__(self):
        self.dataset_length = len(self.selected_folders)
        return (self.dataset_length * self.augment_factor)

    def __getitem__(self, index):
        folder_index = int(index / self.augment_factor)
        file_index = index - (folder_index * self.augment_factor)

        data_folder = self.sequence_dir + self.selected_folders[folder_index]

        pca = torch.load(data_folder + '/' + 'pca/' + f"{file_index:03d}.pt")
        tsr = torch.load(data_folder + '/' + 'tsr/' + f"{file_index:03d}.pt")

        gt_mask_bgr = cv2.imread(self.gt_dir + self.selected_folders[folder_index].upper() + '.png')
        gt_mask = cv2.cvtColor(gt_mask_bgr, cv2.COLOR_BGR2GRAY) / 255.0
